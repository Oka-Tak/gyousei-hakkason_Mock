...
import type { GraphNodeDatum } from '../../hooks/useGraphData';
...
const NodeDetails: React.FC<NodeDetailsProps> = ({ node, isMobile = false, onClose, loadingSpending }) => {
  ...
  return (
    <div style={isMobile ? {
      position: 'fixed', bottom: 0, left: 0, right: 0, background: '#fff', borderTop: '1px solid #ccc', borderRadius: '12px 12px 0 0', padding: 16, zIndex: 102, maxHeight: '60vh', overflowY: 'auto', boxShadow: '0 -4px 16px rgba(0,0,0,0.1)'
    } : {
      position: 'absolute', top: 60, right: 20, zIndex: 3, background: '#fff', border: '1px solid #ccc', padding: 12, minWidth: 320, maxWidth: 420, borderRadius: 8, boxShadow: '0 2px 8px rgba(0,0,0,0.1)'
    }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
        <h2 style={{ margin: 0, fontSize: isMobile ? 18 : 20 }}>{node.name}</h2>
        {isMobile && (
          <button onClick={onClose} style={{ background: 'none', border: 'none', fontSize: 24, cursor: 'pointer', padding: 4, color: '#666' }}>×</button>
        )}
      </div>
