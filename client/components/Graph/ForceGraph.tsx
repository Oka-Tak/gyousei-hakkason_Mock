...
import type { GraphNodeDatum, GraphLinkDatum } from '../../hooks/useGraphData';
...
const ForceGraph: React.FC<ForceGraphProps> = ({
  ...
  useEffect(() => {
    ...
    if (onZoomReady) onZoomReady(zoom);
    ...
    // add icon path for non-agency nodes
    nodeEnter
      .filter(d => d.group !== 'agency_name')
      .append('path')
  ...
  // Highlight and spotlight focus (persistent until cleared)
  useEffect(() => {
    if (!svgRef.current) return;
    const svg = d3.select(svgRef.current);
    if (focusedNodeId) {
