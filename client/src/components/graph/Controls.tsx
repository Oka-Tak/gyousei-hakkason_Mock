...
const Controls: React.FC<ControlsProps> = (props) => {
  const { isMobile = false, showSpotlight, onOpenSpotlight, onCloseSpotlight, search, onChangeSearch, currentIndex, hitsCount, onPrev, onNext, results, onPickResult, agencies, visibleAgencies, onToggleAgency } = props;
  return (
    <>
      <div style={isMobile ? { position: 'absolute', top: 0, left: 0, width: '100vw', zIndex: 10, background: '#fff', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, padding: '8px 0 2px 0' } : { position: 'absolute', top: 0, left: 0, width: '100%', zIndex: 10, background: '#fff', display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 24, padding: '8px 0' }}>
        <h1 style={{ margin: 0, fontSize: isMobile ? 17 : 24, flex: 'none', color: '#333' }}>ZAIMYAKU</h1>
        <button style={isMobile ? { marginTop: 2, background: '#fff', border: '1px solid #ccc', borderRadius: 8, padding: '6px 16px', fontSize: 15, boxShadow: '0 2px 8px rgba(0,0,0,0.08)', cursor: 'pointer', alignSelf: 'center' } : { marginLeft: 24, background: '#fff', border: '1px solid #ccc', borderRadius: 8, padding: '8px 20px', fontSize: 18, boxShadow: '0 2px 8px rgba(0,0,0,0.08)', cursor: 'pointer' }} onClick={onOpenSpotlight} aria-label="検索を開く">🔍 検索</button>
      </div>
      ...
      {agencies && visibleAgencies && onToggleAgency && (
        <div style={isMobile ? { position: 'absolute', bottom: 10, right: 10, zIndex: 30, background: '#fff', border: '1px solid #ccc', borderRadius: 6, padding: '6px 6px', maxHeight: '30vh', overflowY: 'auto', boxShadow: '0 2px 8px rgba(0,0,0,0.08)', fontSize: 13, minWidth: 90 } : { position: 'absolute', bottom: 30, right: 40, zIndex: 30, background: '#fff', border: '1px solid #ccc', borderRadius: 6, padding: '6px 10px', maxHeight: '50vh', overflowY: 'auto', boxShadow: '0 2px 8px rgba(0,0,0,0.08)', fontSize: 14, minWidth: 120 }}>
          ...
          {agencies.map((agency) => (
            <label key={agency} style={{ display: 'block', marginBottom: 2, fontSize: isMobile ? 13 : 14 }}>
              <input type="checkbox" checked={visibleAgencies.includes(agency)} onChange={e => onToggleAgency(agency, e.target.checked)} style={{ marginRight: 4 }} />
              {agency}
            </label>
          ))}
        </div>
