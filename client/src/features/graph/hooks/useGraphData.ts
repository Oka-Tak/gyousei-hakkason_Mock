...
export function useMainGraphData(visibleAgencies: string[]): GraphDataResult {
  ...
  const { nodes, links, colorMap, limitedNodes } = useMemo(() => {
    ...
    nodes.forEach(n => { const v = aggregatedBudgets.get(n.id) || 0; n.value = v; n.initial_budget = v; });

    const links: GraphLinkDatum[] = [];
    aggregatedLinkBudgets.forEach((targetMap, sourceId) => { targetMap.forEach((budget, targetId) => { const s = nodeMap.get(sourceId); const t = nodeMap.get(targetId); if (s && t) links.push({ source: s, target: t, value: budget }); }); });
  ...
  const { nodes, links, colorMap, limitedNodes } = useMemo(() => {
    ...
    filtered.forEach(item => {
      ...
      hierarchy.forEach((name, index) => {
        ...
        if (!nodeMap.has(id)) {
          const newNode: GraphNodeDatum = { id, name: String(name), yomi: nodeYomi, group, value: 0, initial_budget: item.initial_budget_total || 0, url: '#', topLevel, project_id: isLast ? String(item.project_id ?? '') : undefined, spending_list: item.spending_list || [] };
          nodes.push(newNode);
