        ...
        onPickResult={async (id, index) => {
          ...
          if (n) {
            ...
            if (n.group === 'project_name' && (!n.spending_list || n.spending_list.length === 0) && n.project_id) {
              try { setLoadingSpending(true); const res = await fetch(`/api/project_spending?projectId=${encodeURIComponent(String(n.project_id))}`); if (res.ok) { const json = await res.json(); const list = json.spending_list || []; setSelectedNode({ ...n, spending_list: list }); } } finally { setLoadingSpending(false); }
            }
          }
        }}
      />
      ...
      <ForceGraph
        ...
        onNodeClick={async (d) => {
          ...
          if (d.group === 'project_name' && (!d.spending_list || d.spending_list.length === 0) && d.project_id) {
            try { setLoadingSpending(true); const res = await fetch(`/api/project_spending?projectId=${encodeURIComponent(String(d.project_id))}`); if (res.ok) { const json = await res.json(); const list = json.spending_list || []; setSelectedNode({ ...d, spending_list: list }); } } finally { setLoadingSpending(false); }
          }
        }}
